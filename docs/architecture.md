...
Server-rendered Jinja pages are autoescaped and remain a thin transport/presentation adapter. M12C presents a persisted alert queue, source and assessed severity separately, advisory model confidence, escalation, policy metadata, evidence provenance, tool chronology and reviewer-visible `evidence_goal`, catalog-owned action metadata, exact approval binding, simulated execution, and sanitized audit timelines. Browser triage, approval, rejection, and simulation are POST-only, require server-side role authorization, and validate a session-bound CSRF token before invoking the existing orchestrator or approval application service. GET remains non-mutating. Templates cannot select reasoners, prompts, tools, policy, action risk, approval requirements, or execution implementations and never access ORM sessions or adapters directly.
