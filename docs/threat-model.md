...
`openai-l1-v2` exposes alert-derived authorized target identifiers and catalog-derived action semantics as advisory model context. The model cannot add scope or alter catalog policy: gateway and deterministic policy re-read their own authoritative state. V2 also neutralizes artificial benchmark cues only in its provider-facing projection. This reduces known evaluation leakage but creates a presentation-transformation risk, mitigated by narrow deterministic replacements, preservation tests for security facts, and retention of the original normalized/audited data. V1 remains frozen and unchanged.

M12A treats process interruption as an explicit failure mode. A trusted operator command fails stale `RUNNING` triage and stale `EXECUTING` simulations with durable correlated audit records; it never infers provider/action success or automatically reruns work. Repeated recovery is idempotent. Production configuration now fails closed before composition because production authentication and tenant isolation are absent.

Local `.env.local` secrets are excluded from Git and the Docker build context. The future Azure trust path is workload Managed Identity to Key Vault to process configuration; no long-lived Azure client secret is needed merely to obtain the provider key. Residual deployment risks—rate limiting, TLS termination, host controls, PostgreSQL access controls, backup/retention, external audit export, and identity integration—remain prerequisites for production use.
...
| Partial or duplicate processing | Conflicting results/actions or provider-controlled identifier collisions | Idempotency keys; application-generated canonical tool-invocation and action IDs; unique constraints; atomic invocation/audit commits; staged sanitized failure recovery; explicit lifecycle; immutable terminal results | Retry, cross-execution duplicate proposal/recommendation, fault-injection, and concurrency tests |
